...
| Procure inputs (feed, in a dry season) | High uncertainty on feed availability/price exactly when it matters most | `[VALIDATE]` — not yet mapped |
| Sell/output | Sale timing/channel decision under real income-volatility stakes, without necessarily having clear market-timing signals | `[VALIDATE]` — checks "several sources" is a plausible but unconfirmed guess, by analogy with general adoption-barrier literature |
